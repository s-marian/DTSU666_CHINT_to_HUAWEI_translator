...
*/
#define HUAWEI_ID 0x0A			// must be this same address as in setup Huawey inverter
...
#define READ_TIMER 500			// set up how often read the CHINT Power Meter
...
int HuaweiTranslate[CHINT_REQUEST1 / 2 + CHINT_REQUEST2 / 2] = { 6, 7, 8,	// Phase A,B,C current   0 1 2
	24,							// 3  ???
...
int Divider[CHINT_REQUEST1 / 2 + CHINT_REQUEST2 / 2] = { 10, 10, 10,	// Phase A-B,B-C,C-A Voltage  0 1 2
	...
	1000, 1000, 1000, 1000,		// Power Factor total , phase A, B, C 21,22, 23, 24
	1, 1, 1,					// 25, 26, 27   ???
	1,							// 28                   ???
	1, 1, 1,					// 29 30 31             ???
	1,							// 32                   ???
...
ModbusClientRTU MbChint (Serial1, REDE1_PIN, 6);
...
void setup () {
	...
	MbChint.setTimeout (500);	// Necessary for avoiding packets stack up in the Modbus queue
	...
	for (int i = 1; i <= 247; i++) {
		MBserver.registerWorker (i, READ_HOLD_REGISTER, &FC03);	// Register served function code worker for server 11, FC 0x03
	}
	Serial.println ("MBserver register worker OK!");
	MBserver.start (0);			// Start ModbusRTU background task on CPU 1
...
void loop () {
	...
	if (millis () - next_request > READ_TIMER) {
		...
		Error err = MbChint.addRequest (millis (), CHINT_ID, READ_HOLD_REGISTER,
										CHINT_START_REG_1, CHINT_REQUEST1);
		if (err == SUCCESS) {
	...
	if (data_ready) {
		data_ready = false;
		for (uint8_t j = 0; j < (CHINT_REQUEST1 + CHINT_REQUEST2) / 2 * 0; ++j)
			Serial.printf ("j=%i      LoadresCHINT=%i;      wart=%8.4f: \n", j, (2 * j + CHINT_START_REG_1), Chint_RegData[j]);
	}
